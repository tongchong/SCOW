...
- 5cc5ef9: 修复账户管理员在账户管理页面搜索账户充值记录时，除首次搜索外丢失 accountName 参数名导致接口报错无权限问题
- 9c5c3c1: 租户列表余额展示精确到 3 位小数，租户充值时金额只精确到 2 位小数
- 70ab2e1: 修复用户空间-已结束作业批量搜索的账户条件会带入到精确搜索中
- c3907e8: 管理系统设置价格时允许设置为 0 元
- 858c7a6: 创建用户时备注改为非必填，修复成功时不展示提示的问题
- 2abcb9e: 修复了一个 bug，曾导致当用户修改集群配置后，无法计算出正确的计费项 id
- 977600e: 修复了一个 bug，该 bug 曾导致分页情况下导入的账户没有拥有者
- 8268e34: 修改 DELETE 请求 body 为 query 使其合规
- e2c8049: 修改平台用户列表只能在第一页搜索用户问题；为了与租户管理的用户界面搜索统一，平台管理用户界面修改为模糊搜索
- 8c57515: 平台管理租户充值记录过滤条件 tenantName 手动清空时，传参由“”改为 undefined
- 230a8f0: 修复租户财务人员获取租户下的所有账号信息无权限，平台财务人员访问平台下的所有租户信息无权限问题
- 40b9866: 租户管理页面可以查看作业价格表的历史记录
  - @scow/protos@0.2.1
